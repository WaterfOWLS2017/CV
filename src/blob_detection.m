...
% Method #1: using im2bw()
  ...
  binaryImage = im2bw(grayImage, graythresh(grayImage));       % One way to threshold to binary
% Method #2: using a logical operation.
% thresholdValue = 100;
% binaryImage = grayImage > thresholdValue; % Bright objects will be chosen if you use >.
% ========== IMPORTANT OPTION ============================================================
...
% Variable to find the  largest area
largeBlob = 0;
...
subplot(3, 3, 5);
for k = 1 : numberOfBlobs           % Loop through all blobs.
    pixelValue = impixel(originalImage, centroidsX(k), centroidsY(k))./255;
    name = rgb2name(pixelValue);
    display(pixelValue);
    display(name);
	text(centroidsX(k) + labelShiftX, centroidsY(k), {num2str(k), name}, 'FontSize', textFontSize, 'FontWeight', 'Bold');
end
