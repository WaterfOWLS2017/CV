...
{    
    ...
    if (nrhs != 1)
    {
        mexErrMsgTxt("Incorrect number of inputs. Function expects one input.");
    }
    
    // Check input dimensions
    fdims = mxGetDimensions(prhs[0]);
...
{  
  ...
  for (int i = 0; i < contours.size(); i++) {
	...
	if (approx.size() == 3) {
        ...
        if (std::abs(1 - ((double)r.width / r.height)) <= 0.4 && std::abs(1 - (area / (CV_PI * (radius*radius)))) <= 0.2) {
  ...
  plhs[0] = ocvMxArrayFromImage_uint8(bw);
      
  // Put the data back into the output MATLAB array
  plhs[1] = ocvMxArrayFromImage_uint8(dst);
  
  // output the variable if there was a circle
  plhs[2] = ocvMxArrayFromImage_uint8(circ);
